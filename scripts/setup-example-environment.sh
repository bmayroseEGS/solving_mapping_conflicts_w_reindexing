...
################################################################################
# Create Data Stream with Mapping Conflict
################################################################################
print_header "Creating Data Stream with Mapping Conflict"

echo "Step 1: Creating data stream with first backing index (keyword mapping)..."

# Ingest data to create data stream - string values will trigger keyword mapping
for i in {1..5}; do
...
# Add the second backing index to the data stream
curl -s -X POST -u "$ELASTICSEARCH_USER:$ELASTICSEARCH_PASSWORD" \
...
echo "Step 3: Ingesting documents into data stream (will go to second backing index)..."

# Ingest data to the data stream - it will route to the write index (second backing index)
# Using numeric values (not strings) so they use the long mapping
for i in {6..10}; do
...
if [ "$LOG_OFFSET_TYPE" != "$SECOND_LOG_OFFSET_TYPE" ]; then
    ...
    echo "    log.offset type: $LOG_OFFSET_TYPE (from dynamic mapping)"
    echo ""
    echo "  Second index ($SECOND_INDEX):"
    echo "    log.offset type: $SECOND_LOG_OFFSET_TYPE (correct per ECS)"
    echo ""
    echo "This matches the blog scenario where:"
    echo "  • Data arrived BEFORE mappings were properly defined"
    echo "  • Elasticsearch used dynamic templates (mapped as keyword)"
    echo "  • Later indices have correct mapping (long)"
    echo ""
    echo "This conflict causes issues with:"
    echo "  • Kibana data views showing a warning icon (⚠️)"
